...
<article class="work-basic-layout-body">
	<section class="wall-page">
		...
		<div class="group-request-page">
			<div class="group-request-page-header">
				<form action="/work/group/${WorkGroup.seq}/request" method="get" id="searchForm">
					<input type="hidden" name="searchOpt" value="${searchOpt}">
					<input type="hidden" name="status" value="${status}">
					<div class="search-user">
						<button type="button" onclick="submitSearchForm('', '${status}')" class="ra-button ${empty searchOpt? 'ra-button--accent':''}">전체</button>
						<button type="button" onclick="submitSearchForm('worker', '${status}')" class="ra-button ${searchOpt eq 'worker'? 'ra-button--accent':''}">내가 받은</button>
						<button type="button" onclick="submitSearchForm('register', '${status}')" class="ra-button ${searchOpt eq 'register'? 'ra-button--accent':''}">내가 요청한</button>
					</div>
					<div class="search-status">
						<button type="button" onclick="submitSearchForm('${searchOpt}', '')" class="ra-button ${empty status? 'ra-button--accent':''}">전체</button>
						<button type="button" onclick="submitSearchForm('${searchOpt}', 'request')" class="ra-button ${status eq 'request'? 'ra-button--accent':''}">요청</button>
						<button type="button" onclick="submitSearchForm('${searchOpt}', 'process')" class="ra-button ${status eq 'process'? 'ra-button--accent':''}">진행</button>
						<button type="button" onclick="submitSearchForm('${searchOpt}', 'complete')" class="ra-button ${status eq 'complete'? 'ra-button--accent':''}">완료</button>
					</div>
				</form>
			</div>
			
			<div class="group-request-page-body">
				<c:choose>
					<c:when test="${fn:length(WorkBoardList) eq 0}">
						<div class="wall-list-empty">
							<h1>등록된 게시물이 없습니다.</h1>
						</div>
					</c:when>
					<c:otherwise>
						<div class="wall-list-wrap">
							<c:forEach items="${WorkBoardList}" var="Board" varStatus="board_status">
								<div class="wall-board-item ${fn:length(WorkBoardList) gt limit and board_status.last? 'observed':''}">
...
<form action="" id="pagingForm">
	...
	<input type="hidden" name="status" value="${status}">
	<c:choose>
		<c:when test="${searchOpt eq 'worker'}">
			<input type="hidden" name="worker" value="${member.id}">
		</c:when>
		<c:when test="${searchOpt eq 'register'}">
			<input type="hidden" name="register" value="${member.id}">
		</c:when>
		<c:otherwise>
		</c:otherwise>
	</c:choose>
	...
	<c:forEach items="${GroupMemberList}" var="groupMember">
		<li class="${groupMember.id}" onclick="addRequestBoardWorker(${groupMember.seq},'${groupMember.id}', this);">
			<span class="avatar" url="https://t1.daumcdn.net/agit_resources/images/empty_profile_large.png" subdomain="poozim" style="width: 30px; height: 30px; background-image: url(&quot;https://t1.daumcdn.net/agit_resources/images/empty_profile_large.png&quot;);">
				<c:if test="${WorkGroup.master_seq eq groupMember.seq}">
					<button class="user-type-badge user-type-badge--x-small ${WorkGroup.master_seq eq groupMember.seq? 'user-type-badge--master':''}">
...
function submitSearchForm(searchOpt, status){
	$("#searchForm").find("input[name='searchOpt']").val(searchOpt);
	$("#searchForm").find("input[name='status']").val(status);
