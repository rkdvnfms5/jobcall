...
function checkBoardValue(){
	var board_type = $("#insertBoardForm #type").val();
	...
	if(board_type == 'request'){
		if($.trim($("#insertBoardForm #worker").val()) == ''){
			return false;
		}
	}
...
function getCommentHtml(comment, coverFlag){
	...
	html += '<div class="comment-body-content">' + comment.content + '</div><br>';
...
function getInsertPlainBoardHtml(){
	...
	html += '</div></div></div>';
	html += '<input id="attachFiles" name="attachFiles" type="file" multiple="multiple" class="hide">';
	html += '</div></div>';
	return html;
}
